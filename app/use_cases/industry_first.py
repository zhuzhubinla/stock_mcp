...
from domain.industry.mapper import map_industry, industry_tree
from domain.factor.engine import industry_factors, indicators, supply_demand
from data.repositories import forecast_repo
from analytics.exposure import elasticity_score
...
def analyze(industry_id=None, code=None, name=None, top_n=10):
    ...
    elastic = {}
    for c in comps:
        elastic[c["symbol"]] = round((c.get("exposure_weight") or 0) * 100, 1)
    ...
    return {
        ...
        "indicators": inds,
        "supply_demand": sd,
        "forecasts": [{"indicator": f["indicator_code"], "period": str(f["period"]) if f["period"] else None,
