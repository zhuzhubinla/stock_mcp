...
def get_technical(symbol, days=120, force_refresh=False):
    ...
    ind["summary"] = technical.summarize_technical(ind)
...
def analyze(symbol, force_refresh=False):
    ...
    ind = technical.compute_indicators(hist.get("bars", [])) if hist.get("bars") else {}
    signals = signal.compute_total(quote, ind, news.get("news", []), fundamentals.get("metrics", []))
